...
mongoose.connect("mongodb://localhost/secondScraperHw");
...
// A GET request to scrape reddit/r/worldnews
app.get("/scrape", function(req, res) {
  Article.findByIdAndRemove().exec();
...
app.get("/articles", function(req, res) {
  ...
  Article.find({}, function(err,docs) {
    if(!err) {
      ...
      res.send("Error retrieving articles " + err);
    }
  })
  ...
  newNote.save(function(err,doc) {
    if(err) {
      ...
      .exec(function(err,doc) {
        if(err) {
          console.log("Error on request for article: " + req.params.id + " with error " + err);
        } else {
          res.json(doc);
        }
      });
